...
const ARROW_KNOCKBACK_IMPULSE = 15;
const ARROW_LIFESPAN_AFTER_HIT = 1000;
const BOX_FRICTION = 0.94;
const BOX_PUSH_FORCE = 0.7;
const BOX_COLLISION_DAMPING = 0.9;
const BOX_SLIDE_FRICTION = 0.01;
const ANGULAR_FRICTION = 0.85;
const TORQUE_FACTOR = 0.0002;
const FORCE_DRUM_MULTIPLIER = 0.1;
const FORCE_DRUM_COLLISION_FORCE = 1.1;
const ZOMBIE_SPEED_BOOST = 1.30;
const ZOMBIE_PUSH_MODIFIER = 0.4;
...
const TRAP_SIZE = 50;
const UNMOVABLE_FURNITURE = ['atm', 'car'];
const WALL_PUSH_OUT_FORCE = 0.01;
...
    switch (obj.id) {
        case 'car': return 20;
        case 'big_table': return 8;
        case 'box': return 5;
        case 'small_bed': return 4;
        case 'small_table': return 2;
        default: return 1;
    }
}
...
// ADICIONADO: Função de colisão para Círculo vs. Retângulo Rotacionado (OBB)
...
        const player = gameState.players[id];
        ...
        if (player.inventory && player.inventory.id === 'Drum') {
            const drumRadius = 10; // O raio (tamanho) da hitbox

            // 1. Calculamos as coordenadas LOCAIS (relativas ao jogador) do centro da sprite do Drum.
            //    Esses números são baseados em como a sprite é desenhada no arquivo game.js.
            const localX = player.width / 2 + 40; // Distância para a FRENTE do jogador
            const localY = 0;                     // Alinhamento vertical (no centro)

            // 2. Pegamos a posição e rotação atuais do jogador.
            const playerCenterX = player.x + player.width / 2;
            const playerCenterY = player.y + player.height / 2;
            const rot = player.rotation;

            // 3. Convertemos a posição local da sprite (passo 1) para a sua posição real no mundo do jogo.
            ...
            // 4. Criamos a hitbox circular nesta posição exata.
            const drumCircleHitbox = { cx: hitbox_cx, cy: hitbox_cy, radius: drumRadius };
            player.drumHitbox = drumCircleHitbox; // Enviamos para o cliente para que ela seja visível

            // O resto do código usa essa hitbox para checar as colisões.
...
                // MODIFICADO: Bloco inteiro de colisão com paredes
                if (mtv) {
                    // Em vez de corrigir a posição instantaneamente, aplicamos uma força contínua
                    // para empurrar o objeto para fora da parede, criando um efeito de deslizamento.
                    item1.vx -= mtv.x * WALL_PUSH_OUT_FORCE;
                    item1.vy -= mtv.y * WALL_PUSH_OUT_FORCE;

                    // Amortecemos a velocidade para evitar que o objeto "grude" na parede e trema,
                    // mas apenas se ele estiver se movendo em direção à parede.
