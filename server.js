...
const BOX_PUSH_FORCE = 0.4; // CORREÇÃO #3 (anterior): Força de empurrar reduzida
const BOX_COLLISION_DAMPING = 0.4;
const BOX_SLIDE_FRICTION = 0.2; 
const ANGULAR_FRICTION = 0.85;
const TORQUE_FACTOR = 0.0002;
const FORCE_DRUM_MULTIPLIER = 0.1;
const FORCE_DRUM_COLLISION_FORCE = 0.4;
const ZOMBIE_SPEED_BOOST = 1.10;
const ZOMBIE_PUSH_MODIFIER = 0.1; 
...
let nextTrapId = 0;
let nextUniqueObjectId = 0; // Adicionado para dar IDs únicos aos objetos

function initializeGame() {
    nextUniqueObjectId = 0; // Reseta o contador de ID a cada novo jogo
    gameState = {
        players: {},
        arrows: [],
        drones: {},
        ...
        box: [{
            id: 'box',
            uniqueId: nextUniqueObjectId++, // CORREÇÃO #1: Adiciona ID único
            x: 2900,
            y: 1150,
            width: 192,
            height: 192,
        ...
        ducts: [{ x: 3150, y: 480, width: 80, height: 80 }, { x: 270, y: 1670, width: 80, height: 80 }, { x: 2450, y: 300, width: 80, height: 80 }, { x: 3940, y: 1440, width: 80, height: 80 }, { x: 2070, y: 1650, width: 80, height: 80 }],
        sunshades: [{ x: 4350, y: 600, width: 320, height: 340 }, { x: 4440, y: 1400, width: 320, height: 340 }],
...
function getVertices(rect) {
    ...
    const points = [{ x: -halfWidth, y: -halfHeight }, { x: halfWidth, y: -halfHeight }, { x: halfWidth, y: halfHeight }, { x: -halfWidth, y: halfHeight }];
...
function updateGameState() {
    ...
    gameState.illusions = gameState.illusions.filter(illusion => {
        if (now - illusion.creationTime > ILLUSION_LIFESPAN) { return false; }
    ...
    for (const id in gameState.players) {
        ...
        if (player.inventory && player.inventory.id === 'forceDrum') {
            const itemWidth = 50;
            const itemHeight = 50;
            const distanceFromPlayer = player.width / 2 + itemWidth / 3;
            const itemCenterX = player.x + player.width / 2 + Math.cos(player.rotation) * distanceFromPlayer;
            const itemCenterY = player.y + player.height / 2 + Math.sin(player.rotation) * distanceFromPlayer;
            const forceDrumHitbox = { x: itemCenterX - itemWidth / 2, y: itemCenterY - itemHeight / 2, width: itemWidth, height: itemHeight, rotation: player.rotation };
            for (const obj of allCollidables) {
                const mtv_drum_object = checkCollisionSAT(forceDrumHitbox, obj);
                if (mtv_drum_object) {
                    let isObjectTouchingWall = false;
                    for (const wall of allWalls) { if (checkCollisionSAT(obj, wall)) { isObjectTouchingWall = true; break; } }
                    if (isObjectTouchingWall) { obj.ignoreWallCollision = true; }
                    ...
                    const contactVectorX = itemCenterX - (obj.x + obj.width / 2);
                    const contactVectorY = itemCenterY - (obj.y + obj.height / 2);
    ...
    for (let i = 0; i < allCollidables.length; i++) {
        ...
        for (let j = i + 1; j < allCollidables.length; j++) {
            ...
            if (mtv) {
                ...
                if (mtvLength > 0.01) {
                    ...
                    item1.x -= normalX * mtvLength / 2;
                    item1.y -= normalY * mtvLength / 2;
                    item2.x += normalX * mtvLength / 2;
                    item2.y += normalY * mtvLength / 2;
                    ...
                    if (velAlongNormal < 0) {
                        ...
                        const jn = -(1 + restitution) * velAlongNormal / 2;
                        ...
                        item1.vx -= (impulseNormalX + impulseTangentX);
                        item1.vy -= (impulseNormalY + impulseTangentY);
                        item2.vx += (impulseNormalX + impulseTangentX);
                        item2.vy += (impulseNormalY + impulseTangentY);
                    }
                }
            }
        }
        ...
        if (!item1.ignoreWallCollision) {
            for (const obstacle of allWalls) {
                ...
                if (mtv) {
                    ...
                    if (lenSq > 0) {
                        ...
                        item1.vx = reflectionX * BOX_COLLISION_DAMPING;
                        item1.vy = reflectionY * BOX_COLLISION_DAMPING;
                    }
    ...
    for (const id of playerIds) {
        ...
        player.hitbox = { cx: player.x + player.width / 2, cy: player.y + player.height / 2, radius: player.width / 2 };
        
        // NOVA CORREÇÃO: Reduz a hitbox física do humano
        let hitboxRadius = player.width / 2;
        if (player.role === 'human') {
            hitboxRadius *= 0.3; // Reduz a hitbox do humano para 80% do seu tamanho visual
        } else if (player.role === 'zombie') {
            hitboxRadius *= 0.6; // Reduz a hitbox do zombie  para 80% do seu tamanho visual
        }

        player.physicalHitbox = { cx: player.x + player.width / 2, cy: player.y + player.height / 2, radius: hitboxRadius };
        
        player.infectionHitbox = { cx: player.x + player.width / 2, cy: player.y + player.height / 2, radius: player.width * 0.2 };
        ...
        if (player.inventory && player.inventory.id === 'skateboard') {
            ...
            let collidedX = false;
            for (const wall of allWalls) { if (isCollidingCircleRect(player.physicalHitbox, wall)) { collidedX = true; } }
            ...
            let collidedY = false;
            for (const wall of allWalls) { if (isCollidingCircleRect(player.physicalHitbox, wall)) { collidedY = true; } }
            ...
            if (player.carryingObject) { effectiveSpeed *= 0.7; }
        ...
        const playerPoly = { x: player.physicalHitbox.cx - player.physicalHitbox.radius, y: player.physicalHitbox.cy - player.physicalHitbox.radius, width: player.physicalHitbox.radius * 2, height: player.physicalHitbox.radius * 2, rotation: 0 };
        ...
        for (const item of allCollidables) {
            ...
            if (mtv) {
                ...
                if (player.inventory && player.inventory.id === 'skateboard') { continue; }
                ...
                if (isPushing) {
                    ...
                    if (player.inventory && player.inventory.id === 'forceDrum') { pushMultiplier = FORCE_DRUM_MULTIPLIER; }
                    if (player.role === 'zombie') { pushMultiplier *= ZOMBIE_PUSH_MODIFIER; }
                    ...
                    // CORREÇÃO #4 (anterior): Não aplicar torque se o objeto estiver colidindo com paredes/objetos.
    ...
    for (let i = gameState.traps.length - 1; i >= 0; i--) {
        ...
        if (trapped) { gameState.traps.splice(i, 1); }
    }
                ...
                for (const id2 of playerIds) {
                    ...
                    if ((player2.role === 'human' || player2.isSpying) && isCollidingCircleCircle(player1.hitbox, player2.hitbox) && !player2.isFlying && !player2.isTrapped) {
                        ...
                        if (player2.activeAbility === 'butterfly' && !player2.butterflyUsed) {
                            ...
                            setTimeout(() => { if (gameState.players[id2]) { const p = gameState.players[id2]; p.isFlying = false; p.speed = p.originalSpeed; } }, BUTTERFLY_DURATION);
                        ...
                        if (player2.inventory && player2.inventory.id === 'invisibilityCloak') {
                            ...
                            if (player2.isInvisible) { player2.isInvisible = false; }
                        } else {
                        ...
                        if (player2.isSpying) { player2.isSpying = false; }
                        player2.role = 'zombie';
                ...
                for (const playerId in gameState.players) {
                    ...
                    if (isCollidingCircleRect(player.hitbox, arrowHitbox)) {
                        ...
                        setTimeout(() => {
                            ...
                            if (index !== -1) { gameState.arrows.splice(index, 1); }
...
io.on('connection', (socket) => {
    ...
    socket.on('buyRareItem', (itemId) => {
        const player = gameState.players[socket.id];
        if (!player || !player.inventory || player.inventory.id !== 'card') { return; }
        let cost;
        let itemData;
        switch (itemId) {
            case 'skateboard':
        ...
        if (cost && player.coins >= cost) {
            ...
            io.emit('newMessage', { name: 'Server', text: `${player.name} used the ATM to buy ${itemId}!` });
        }
    });
        ...
        switch (actionData.type) {
            case 'use_antidote':
                if (player.inventory && player.inventory.id === 'antidote') {
                    player.inventory = null;
                    io.emit('newMessage', { name: 'Server', text: `${player.name} consumed an antidote.` });
                }
                break;
            ...
            case 'drop_grenade':
                if (player.inventory && player.inventory.id === 'drone' && gameState.drones[player.id] && gameState.drones[player.id].ammo > 0) {
                    ...
                    gameState.grenades.push({ id: nextGrenadeId++, x: drone.x, y: drone.y, explodeTime: Date.now() + GRENADE_FUSE_TIME });
                }
                break;
            case 'primary_action':
                if (player.activeAbility === 'archer' && player.arrowAmmo > 0) {
                    player.arrowAmmo--;
                    gameState.arrows.push({ id: nextArrowId++, x: player.x + player.width / 2, y: player.y + player.height / 2, width: 30, height: 30, color: 'red', angle: player.rotation, ownerId: player.id, hasHit: false });
                }
                break;
            case 'ability':
                ...
                if (player.activeAbility === 'illusionist' && player.illusionistAvailable) {
                    ...
                    gameState.illusions.push({ id: nextIllusionId++, x: player.x, y: player.y, width: player.width, height: player.height, vx: Math.cos(player.rotation) * ILLUSION_SPEED, vy: Math.sin(player.rotation) * ILLUSION_SPEED, creationTime: Date.now() });
                    setTimeout(() => { if (gameState.players[socket.id]) player.illusionistAvailable = true; }, ILLUSIONIST_COOLDOWN);
                }
                if (player.activeAbility === 'athlete' && player.sprintAvailable) {
                    ...
                    setTimeout(() => { if (gameState.players[socket.id]) { player.isSprinting = false; player.speed = originalSpeed; } }, SPRINT_DURATION);
                    setTimeout(() => { if (gameState.players[socket.id]) player.sprintAvailable = true; }, SPRINT_COOLDOWN);
                }
                ...
                if (player.carryingObject) {
                    if (player.inventory && player.inventory.id === 'gravityGlove') {
                        ...
                        if (player.inventory.uses <= 0) { 
                            player.inventory = null; 
                            io.emit('newMessage', { name: 'Server', text: `${player.name}'s Gravity Glove broke!` });
                        }
                    }
                    ...
                    if (player.inventory.id === 'invisibilityCloak' && player.inventory.active) { return; }
                    dropHeldItem(player);
                }
                break;
            case 'interact':
...
function startNewRound() {
    ...
    const cardSpawnLocations = [{ x: 3500, y: 1500 }];
    for (let i = 0; i < 1; i++) {
        const spawnPoint = cardSpawnLocations[i];
        gameState.groundItems.push({ id: 'card', x: spawnPoint.x, y: spawnPoint.y, width: 37, height: 25, });
    }
    ...
    if (gameState.gamePhase === 'waiting') {
        ...
        for (const id of playerIds) {
            if (gameState.players[id].role === 'zombie') hasZombies = true;
             // CORREÇÃO #5 (anterior): Jogadores espiões não contam como humanos.
