...
const BOX_FRICTION = 0.98;         // Menos atrito, desliza mais
const BOX_PUSH_FORCE = 1.5;        // Empurrão inicial mais forte
const BOX_COLLISION_DAMPING = 0.5; // Colisões mais "vivas"
const ANGULAR_FRICTION = 0.92;     // O giro diminui mais suavemente
const TORQUE_FACTOR = 0.002;       // Menos força de rotação ao empurrar
...
function initializeGame() {
    gameState = {
        ...
        box: [
            { id: 'box1', x: 2720, y: 1200, width: 90, height: 90, vx: 0, vy: 0, rotation: 0, angularVelocity: 0 },
            { id: 'box2', x: 2900, y: 1150, width: 192, height: 192, vx: 0, vy: 0, rotation: 300, angularVelocity: 0 },
            { id: 'box3', x: 800, y: 300, width: 90, height: 90, vx: 0, vy: 0, rotation: 0, angularVelocity: 0 },
            { id: 'box4', x: 1700, y: 700, width: 128, height: 128, vx: 0, vy: 0, rotation: 45, angularVelocity: 0 },
        ],
        furniture: [
        ...
        atm: { x: 2890, y: 870, width: 120, height: 150 },
        ducts: [
...
function updateGameState() {
    ...
    gameState.illusions = gameState.illusions.filter(illusion => {
        ...
        const obstacles = [...gameState.house.walls, ...gameState.garage.walls, gameState.atm];
    ...
    for (const id in gameState.players) {
        ...
        if (player.inventory && player.inventory.id === 'skateboard') {
            ...
            if (isCollidingCircleRect(player.physicalHitbox, gameState.atm)) { collidedX = true; }
            ...
            if (isCollidingCircleRect(player.physicalHitbox, gameState.atm)) { collidedY = true; }
            ...
            if (mtv) {
                ...
                if (isPushing) {
                    ...
                    if (player.inventory && player.inventory.id === 'heavyGloves' && item.id !== 'car') {
                        const staticObstacles = [...gameState.house.walls, ...gameState.garage.walls, gameState.atm];
                    ...
                    if ((player2.role === 'human' || player2.isSpying) && isCollidingCircleCircle(player1.hitbox, player2.hitbox) && !player2.isFlying) {
                        ...
                        if (player2.inventory && player2.inventory.id === 'antidote') {
                            ...
                            if (Math.random() < immunityProbability) {
                                io.emit('newMessage', { name: 'Server', text: `${player2.name} resisted the infection with a pill!` });
                                continue;
                            } else {
                                io.emit('newMessage', { name: 'Server', text: `${player2.name}'s pill failed to grant immunity!` });
                            }
                        }
            ...
            if (humanCount === 0 && playerIds.length > 0) {
                ...
                for (const id in gameState.players) {
                    ...
                    if (savedData) {
                        ...
                        if (player.inventory && player.inventory.id === 'boxing_glove') {
                            player.inventory = null;
                        }
...
io.on('connection', (socket) => {
    ...
    socket.on('setNickname', (nickname) => {
        ...
        if (player && nickname) {
            ...
            if (sanitizedName) {
                ...
                io.emit('newMessage', { name: 'Server', text: `${player.name} has joined the game.` });
            }
        }
    });
    ...
    socket.on('chooseAbility', (ability) => {
        ...
        if (player && player.activeAbility === ' ' && cost !== undefined && player.coins >= cost) {
            ...
            if (ability === 'fighter') {
                player.inventory = { id: 'boxing_glove' };
            }
        }
    });
    ...
    socket.on('buyItem', (itemId) => {
        ...
        if (itemId === 'card') {
            if (gameState.atmCardsSold >= 3) {
                io.to(socket.id).emit('newMessage', { name: 'Server', text: 'All ATM cards for this match have been sold.' });
                return;
            }
            cost = 100;
            itemData = { id: 'card' };
        ...
        if (actionData.type === 'primary_action') {
            if (player.activeAbility === 'fighter' && Date.now() > player.lastPunchTime) {
                ...
                if (closestZombie) {
                    ...
                    const obstacles = [...gameState.house.walls, ...gameState.garage.walls, ...gameState.box, ...gameState.furniture, gameState.atm];
                    const zombiePoly = { x: closestZombie.x, y: closestZombie.y, width: closestZombie.width, height: closestZombie.height, rotation: 0 };
                    for(const obstacle of obstacles) {
                    ...
                    if (!isAlreadyStuck) {
                        const newZombiePoly = { x: closestZombie.x, y: closestZombie.y, width: closestZombie.width, height: closestZombie.height, rotation: 0 };
                        for(const obstacle of obstacles) {
